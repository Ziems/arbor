...
from .logging import get_logger, request_id_context, job_id_context, user_context
...
class ArborException(Exception):
    ...
    def _log_error(self):
        ...
        if self.severity == ErrorSeverity.CRITICAL:
            logger.critical(f"Critical error: {self.message}", log_context, exc_info=True)
        elif self.severity == ErrorSeverity.HIGH:
            logger.error(f"High severity error: {self.message}", log_context, exc_info=True)
...
class ErrorHandler:
    ...
    def _categorize_exception(self, exc: Exception) -> tuple[ErrorCategory, ErrorSeverity]:
        ...
        elif any(keyword in exc_type for keyword in ["memory", "resource", "gpu", "cuda"]):
        ...
        elif any(keyword in exc_type for keyword in ["network", "connection", "timeout"]):
