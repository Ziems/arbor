    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if self.trainer and state.global_step != self.trainer._last_loaded_step:
            ...
            if self.comms_handler and self.comms_handler.is_main_process:
                ...
                logger.info("Waiting for server to confirm inference requests are complete...")
                max_wait_time = 60  # Maximum time to wait for server response
                wait_start = time.time()
                
                while self._waiting_for_ready:
                    if time.time() - wait_start > max_wait_time:
                        logger.warning(f"Timeout waiting for server ready signal after {max_wait_time}s, proceeding anyway...")
                        break
                    time.sleep(0.1)  # Check frequently for server response
                
                logger.info("Server ready, all processes will now perform weight update...")
            ...
            logger.info(f"Process rank {getattr(self.trainer.accelerator, 'process_index', 'unknown')} - About to call _move_model_to_vllm() for step {state.global_step}")
            
            try:
                import signal
                
                def timeout_handler(signum, frame):
                    raise TimeoutError("_move_model_to_vllm() call timed out after 120 seconds")
                
                # Set a 2-minute timeout for the weight update
                signal.signal(signal.SIGALRM, timeout_handler)
                signal.alarm(120)
                
                logger.info(f"Process rank {getattr(self.trainer.accelerator, 'process_index', 'unknown')} - Starting _move_model_to_vllm() call...")
                start_time = time.time()
                self.trainer._move_model_to_vllm()
                end_time = time.time()
                logger.info(f"Process rank {getattr(self.trainer.accelerator, 'process_index', 'unknown')} - Successfully completed _move_model_to_vllm() call in {end_time - start_time:.2f} seconds")
                ...
                signal.alarm(0)
                logger.error(f"Process rank {getattr(self.trainer.accelerator, 'process_index', 'unknown')} - Error during _move_model_to_vllm(): {e}")
