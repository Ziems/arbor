...
class InferenceJob(Job):
    ...
    def launch(self, model: str, launch_config: InferenceLaunchConfig):
        ...
        memory_util = launch_config.gpu_memory_utilization or 0.9
        command = f"{sys.executable} -m {vllm_module} --model {self.launched_model_name} --port {self.port} --gpu-memory-utilization {memory_util} --data-parallel-size {n_gpus} --enable_prefix_caching"
