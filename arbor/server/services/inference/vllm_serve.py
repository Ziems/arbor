    ...
    while True:
        ...
        if command["type"] in ["call", "fire_and_forget"]:
            ...
            try:
                ...
                # Add specific logging for continue_final_message errors
                if "continue_final_message" in str(e):
                    logger.error(
                        f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: continue_final_message error"
                    )
                    logger.error(
                        f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: Method: {method_name}"
                    )
                    logger.error(
                        f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: Error: {e}"
                    )

                    # Log the arguments passed to the chat method
                    if method_name == "chat" and kwargs:
                        logger.error(
                            f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: Chat method kwargs keys: {list(kwargs.keys())}"
                        )

                        # Log the messages structure
                        if "messages" in kwargs:
                            messages = kwargs["messages"]
                            logger.error(
                                f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: Number of message batches: {len(messages)}"
                            )

                            for batch_idx, message_batch in enumerate(messages):
                                logger.error(
                                    f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: Message batch {batch_idx} ({len(message_batch)} messages):"
                                )
                                for msg_idx, msg in enumerate(message_batch):
                                    content = (
                                        msg.get("content", "")
                                    ...
                                    logger.error(
                                        f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG:   Message {msg_idx}: "
                                        f"role='{msg.get('role', 'unknown') if isinstance(msg, dict) else 'unknown'}', "
                                        f"content_len={len(content)}"
                                    )
                                    logger.error(
                                        f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG:   Content preview: '{content_preview}'"
                                    )
                                    ...
                                    if isinstance(msg, dict):
                                        if not content.strip():
                                            logger.error(
                                                f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG:   ⚠️  ISSUE: Empty/whitespace-only content"
                                            )
                                        if content != content.strip():
                                            logger.error(
                                                f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG:   ⚠️  ISSUE: Leading/trailing whitespace"
                                            )
                                        if (
                                            msg.get("role") == "assistant"
                                            and batch_idx == len(messages) - 1
                                            and msg_idx == len(message_batch) - 1
                                        ):
                                            logger.error(
                                                f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG:   ⚠️  This is the final assistant message that should be continued"
                                            )
                        ...
                        for flag in flags_to_log:
                            if flag in kwargs:
                                logger.error(
                                    f"[WORKER {data_parallel_rank}] CHAT_TEMPLATE_DEBUG: {flag}={kwargs[flag]}"
                                )
