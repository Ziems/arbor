# Need will brown attribution here
...
def main(script_args: ScriptArguments):
    ...
    async def chat(request: ChatRequest):
        ...
        for connection, messages in zip(connections, chunked_messages):
            ...
            # However, vLLM requires that we always send at least one message. So we send a placeholder message to comply
            # with vLLM's requirement, and we later ignore the result.
            if not messages:
        ...
        # Handle empty messages (see above)
        all_outputs = [
            output
