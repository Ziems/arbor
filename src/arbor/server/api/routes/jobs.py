...
@router.get("/{job_id}/checkpoints", response_model=PaginatedResponse[JobCheckpointModel])
def get_job_checkpoints(request: Request, job_id: str):
    job_manager = request.app.state.job_manager
    job = job_manager.get_job(job_id)
    return PaginatedResponse(
        data=[JobCheckpointModel(id=checkpoint.id, fine_tuned_model_checkpoint=checkpoint.fine_tuned_model_checkpoint, fine_tuning_job_id=checkpoint.fine_tuning_job_id, metrics=checkpoint.metrics, step_number=checkpoint.step_number) for checkpoint in job.get_checkpoints()],
        has_more=False
    )
...
def get_job_status(
    ...
    job_manager = request.app.state.job_manager
    job = job_manager.get_job(job_id)
    return JobStatusModel(id=job_id, status=job.status.value, fine_tuned_model=job.fine_tuned_model)
